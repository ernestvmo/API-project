...
def get_training_dir(args, confirm=True):
  ...
  if args.training_sub_dir is not None:
    ...
    training_dir = os.path.join(args.training_base_dir, args.training_sub_dir)
...
def execute(args, strategy=None):
  ...
  try:
    ...
    if args.mode == 'train':
      # Save args
      filepath = os.path.join(args.training_dir, 'args_{0}.txt'.format(datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S")))
