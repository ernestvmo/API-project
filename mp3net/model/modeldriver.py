...
class DistributedModelDriver:
  def __init__(self, strategy, model_factory: mp3net.MP3netFactory, stage, mode, summary_dir, args):
    ...
    with self.strategy.scope():
      ...
      self.checkpoint_dir = os.path.join(args.training_dir, "stage-{}".format(self.model.stage))
  ...
  def write_summary_gan(self, reals_batch_per_replica, step):
    ...
    if self.model.freq_n == self.model.audio_representation.freq_n:
      ...
      if infer_dir is not None and not os.path.isdir(infer_dir):
        os.mkdir(infer_dir)
      ...
      for i in range(3):
        ...
        if infer_dir is not None:
          audio_utils.save_audio(os.path.join(infer_dir, f'fake_sample{i}.wav'), wav_fake[i, :, :].numpy(), sample_rate=self.model.sample_rate, out_format='wav')
          audio_utils.save_audio(os.path.join(infer_dir, f'real_noise_sample{i}.wav'), wav_real_noise[i, :, :].numpy(), sample_rate=self.model.sample_rate, out_format='wav')
  ...
  def inference_loop(self):
    inference_dir = os.path.join(self.args.training_dir, "infer")
