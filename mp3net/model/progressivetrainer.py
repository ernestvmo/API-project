...
class ProgressiveTrainer:
  ...
  def evaluation_loop(self):
    ...
    while True:
      ...
      # set up summary writer (different writer for different stage, since different tensors etc)
      summary_dir = os.path.join(self.args.summary_dir, f"stage-{current_stage}")
  ...
  def _find_latest_stage(self):
    ...
    """
    checkpoint_file_path = os.path.join(self.args.training_dir, "stage-*", "stage-*-ckpt-*.data*")
    checkpoint_files = glob.glob(checkpoint_file_path)
  ...
  def _find_latest_checkpoint(self):
    ...
    stage = self._find_latest_stage()
    if stage is not None:
      checkpoint_file_path = os.path.join(self.args.training_dir, f"stage-{stage}", f"stage-{stage}-ckpt-*.data*")
      checkpoint_files = glob.glob(checkpoint_file_path)
  ...
  def find_data_files(training_stage, data_dir, model_factory):
    ...
    file_pattern = f'*.tfrecord'
    input_file_path = os.path.join(data_dir, file_pattern)
    input_files = glob.glob(input_file_path)[0:]
    ...
    for input_filepath in input_files:
      print("  {}".format(os.path.split(input_filepath)[1]))
