...
def decode_extract_and_batch(
  ...
  dataset = tf1.data.Dataset.from_tensor_slices(fps)
  ...
  def _slice(audio):
    ...
    if slice_randomize_offset:
      start = tf1.random_uniform([], maxval=slice_len, dtype=tf1.int32)
      audio = audio[start:]

    # Extract sliceuences
    audio_slices = tf1.signal.frame(
        audio,
        slice_len,
        slice_hop,
  ...
  def _slice_dataset_wrapper(audio):
    ...
    return tf1.data.Dataset.from_tensor_slices(audio_slices)
  ...
  if prefetch_size is not None:
    ...
    if prefetch_gpu_num is not None and prefetch_gpu_num >= 0:
      dataset = dataset.apply(
          tf1.data.experimental.prefetch_to_device('/device:GPU:{}'.format(prefetch_gpu_num)))
