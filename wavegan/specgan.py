...
def SpecGANGenerator(
  ...
  with tf.compat.v1.variable_scope('z_project'):
    output = tf.compat.v1.layers.dense(output, 4 * 4 * dim * 16)
  ...
  if train and use_batchnorm:
    update_ops = tf.compat.v1.get_collection(tf.GraphKeys.UPDATE_OPS, scope=tf.get_variable_scope().name)
...
def SpecGANDiscriminator(
  ...
  with tf.compat.v1.variable_scope('downconv_0'):
    output = tf.compat.v1.layers.conv2d(output, dim, kernel_len, 2, padding='SAME')
  output = lrelu(output)

  # Layer 1
  # [64, 64, 64] -> [32, 32, 128]
  with tf.compat.v1.variable_scope('downconv_1'):
    output = tf.compat.v1.layers.conv2d(output, dim * 2, kernel_len, 2, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
  ...
  with tf.compat.v1.variable_scope('downconv_2'):
    output = tf.compat.v1.layers.conv2d(output, dim * 4, kernel_len, 2, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
  ...
  with tf.compat.v1.variable_scope('downconv_3'):
    output = tf.compat.v1.layers.conv2d(output, dim * 8, kernel_len, 2, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
  ...
  with tf.compat.v1.variable_scope('downconv_4'):
    output = tf.compat.v1.layers.conv2d(output, dim * 16, kernel_len, 2, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
